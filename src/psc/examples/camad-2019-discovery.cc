...
int main (int argc, char *argv[])
{
  ...
  switch (txProb)
    {
    case 25:
      pDiscFactory.SetDiscTxProbability ("p25");
      break;
    case 50:
      pDiscFactory.SetDiscTxProbability ("p50");
      break;
    case 75:
      pDiscFactory.SetDiscTxProbability ("p75");
      break;
    case 100:
      pDiscFactory.SetDiscTxProbability ("p100");
      break;
    default:
      NS_FATAL_ERROR ( "Wrong transmission probability");
    }
  ...
  for (uint32_t rmDevIdx = 0; rmDevIdx < remoteUeDevs.GetN (); rmDevIdx++)
    {
      ...
      if (discModel == "A")
        ...
        {
          std::cout << " Discovery Model not recognized" << std::endl;
        }
    }

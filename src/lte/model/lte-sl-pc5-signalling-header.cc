...
LteSlPc5SignallingMessageType::SetMessageType (uint8_t msgType)
{
  switch (msgType)
    ...
    case LteSlPc5SignallingMessageType::RemoteUeInfoResponse:
      ...
      break;
    default:
      NS_ASSERT_MSG (false, "Cannot set the Message Type to the input provided");
      break;
    }
}
...
DirectCommunicationRequest::Print (std::ostream &os) const
{
  os << "msgId: " << (uint16_t) m_msgId << " "
     << "seqNum: " << m_seqNum << " userInfoType: " << (uint16_t) m_userInfoType << " userInfo: ";

  for (std::vector<uint8_t>::const_iterator it = m_userInfo.begin (); it != m_userInfo.end (); ++it)
    {
      os << (uint16_t) *it << " ";
    }
...
DirectCommunicationRequest::Serialize (Buffer::Iterator start) const
{
  ...
  i.WriteU8 (m_userInfo.size ());
  for (std::vector<uint8_t>::const_iterator it = m_userInfo.begin (); it != m_userInfo.end (); ++it)
    {
      if (it == m_userInfo.begin ())
        {
          uint8_t spare = 0;
          uint8_t byte3 = (*it << 4) | (spare << 3) | (m_userInfoType);
          i.WriteU8 (byte3);
        }
      else
        {
...
DirectCommunicationRequest::Deserialize (Buffer::Iterator start)
{
  ...
  uint8_t userInfoLength = i.ReadU8 ();

  for (uint8_t index = 0; index < userInfoLength; index++)
    {
      if (index == 0)
        {
          uint8_t b = i.ReadU8 ();
          uint8_t c = (b << 4);
          m_userInfoType = (c >> 4);
          m_userInfo.push_back ((b >> 4) & 0xff);
        }
      else
        {
          m_userInfo.push_back (i.ReadU8 ());
        }
    }
...
void
DirectCommunicationRequest::SetUserInfo (uint8_t userInfoType, std::vector<uint8_t> userInfo)
{
  NS_ASSERT_MSG (( (userInfoType == 1) || (userInfoType == 2) || (userInfoType == 3) ), "Cannot set the User Info Type to the input provided. Please provide 1 for Upper Layer User Info, 2 for PRUK ID, 3 for IMSI"); //Refer 24.334 Section 12.5.1.3
...
DirectSecurityModeCommand::Print (std::ostream &os) const
{
  ...
  if (m_hasUserInfo)
    {
      os << "userInfoType: " << m_userInfoType << " userInfo: ";
      for (std::vector<uint8_t>::const_iterator it = m_userInfo.begin (); it != m_userInfo.end (); ++it)
        {
          os << (uint16_t) *it << " ";
        }
...
DirectSecurityModeCommand::GetSerializedSize (void) const
{
  ...
  if (m_hasUserInfo)
    {
      size += (2 + m_userInfo.size ()); //2 for the type and value
    }
  if (m_hasSignature)
    {
      size += (1 + SignatureSize); //1 for the type
...
DirectSecurityModeCommand::Deserialize (Buffer::Iterator start)
{
  ...
  uint8_t opt_vars_iei = 0;
  uint8_t gpiLength, userInfoLength, b, c, encryptedPayloadLength;
  while (!i.IsEnd ())
    {
      opt_vars_iei = i.ReadU8 ();
      switch (opt_vars_iei)
...
void
DirectSecurityModeCommand::SetUserInfo (uint8_t userInfoType, std::vector<uint8_t> userInfo)
{
  NS_ASSERT_MSG (( (userInfoType == 1) || (userInfoType == 2) || (userInfoType == 3) ), "Cannot set the User Info Type to the input provided. Please provide 1 for Upper Layer User Info, 2 for PRUK ID, 3 for IMSI"); //Refer 24.334 Section 12.5.1.3
  m_hasUserInfo = true;
  m_userInfo = userInfo;

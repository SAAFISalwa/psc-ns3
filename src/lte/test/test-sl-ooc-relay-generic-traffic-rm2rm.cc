...
SlOocRelayGenericTrafficRm2RmTestCase::ChangeUdpEchoClientRemote (Ptr<Node> newRemoteNode, Ptr<UdpEchoClient> app, uint16_t port )
{
  ...
  NS_LOG_INFO ("Changed m_echoServerAddress to " << m_echoServerAddress);
...
SlOocRelayGenericTrafficRm2RmTestCase::DoRun (void)
{
   ...
   * Each initiating Remote UE starts sending traffic 1.00 s after the start of
  ...
  NS_LOG_INFO ("***************************************************************");
  NS_LOG_INFO ("Starting Test with: "
  ...
  double timeBetweenRelayStarts = 1.0 + m_nRemoteUesPerRelay * ((2 + 2 * m_nRemoteUesPerRelay) * 0.04); //s
  ...
  double simTime = startTimeRemote[(m_nRelayUes * m_nRemoteUesPerRelay - 1)] + 1.0 + 10.0; //s
  ...
  for (uint16_t remUeIdx = 1; remUeIdx < remoteUeNodes.GetN (); remUeIdx++)
    {
      ...
      //Start the application 1 s after the remote UE started the relay service
      ...
      Simulator::Schedule (Seconds (1.0 + startTimeRemote[remUeIdx] - 0.100),
  ...
  for (uint32_t remUeIdx = 1; remUeIdx < m_nRelayUes * m_nRemoteUesPerRelay; remUeIdx++)
    {
      NS_LOG_INFO ("remUeIdx " << remUeIdx << " RxPackets " << m_nRxPacketsPerRemoteUe[remUeIdx]);
  ...
  NS_LOG_INFO ("Ratio of Remote UEs with traffic: " << nRemoteFlows << " / " << (m_nRelayUes * m_nRemoteUesPerRelay - 1) << " = " << nRemoteFlows / (m_nRelayUes * m_nRemoteUesPerRelay - 1));
  NS_LOG_INFO ("Ratio of Remote UEs with enough packets: " << nRemoteFlowsWithEnoughPackets << " / " << (m_nRelayUes * m_nRemoteUesPerRelay - 1) << " = " << nRemoteFlowsWithEnoughPackets / (m_nRelayUes * m_nRemoteUesPerRelay - 1));
  ...
  : TestSuite ("sl-ooc-relay-generic-traffic-rm2rm", SYSTEM)
{
  ...
  AddTestCase (new SlOocRelayGenericTrafficRm2RmTestCase (2, 10), TestCase::QUICK);

...
SlInCovrg2Relay1RemoteRegularTestCase::DoRun (void)
{
  ...
  double remoteUeInitXPos = 500.0; //Initial X coordinate of the remote UE
  ...
  {
	  std::ostringstream txWithAddresses; txWithAddresses<< "/NodeList/" << remoteHost->GetId () << "/ApplicationList/0/TxWithAddresses";
...
{
  // LogComponentEnable ("TestSlInCovrg2Relay1RemoteRegular", LOG_LEVEL_ALL);

  //Test regular scenario
  //1 Remote UE detects, connects, and uses any one of the 2 Relay UEs to send and receive traffic (e.g. wns3-2019 type of scenario).
